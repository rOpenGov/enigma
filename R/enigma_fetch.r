...
enigma_fetch <- function(dataset = NULL, select = NULL, search = NULL, 
  where = NULL, conjunction = NULL, sort = NULL, path=NULL, overwrite = TRUE, 
  ...
  if (is.null(path)) path <- 
    file.path(Sys.getenv('HOME'), parse_url(json$export_url)$path)
  
  not_ready <- TRUE
  while (not_ready) {
    Sys.sleep(poll_sleep)
    bin <- GET(json$export_url, write_disk(path = path, 
                                           overwrite = overwrite), ...)
